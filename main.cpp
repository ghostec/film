...
vec3 radiance(const ray &r, int depth, unsigned short *Xi) {
  ...
  if (++depth>5) if (erand48(Xi)<p) f=f*(1/p); else return obj.emission();
  if (obj.refl() == refl_t::diffusal) {  // Ideal refl_t::diffusalUSE reflection
    double r1 = 2 * PI * erand48(Xi), r2 = erand48(Xi), r2s = sqrt(r2);
    vec3 w = nl,
         u = ((fabs(w[0]) > .1 ? vec3(0, 1, 0) : vec3(1, 0, 0)).cross(w))
                 .normalize(),
         v = w.cross(u);
    vec3 d =
    ...
    return obj.emission() + f * radiance(ray(x, d), depth, Xi);
             ...
             refl_t::specular)  // Ideal refl_t::specularULAR reflection
    return obj.emission() +
  ...
  ray reflray(
      ...
      r.direction() -
          n * 2 *
              n.dot(
                  r.direction()));  // Ideal dielectric refl_t::refractiveACTION
...
int main(int argc, char *argv[]) {
  ...
  vec3 cx(w * .5135 / h, 0, 0), cy = (cx.cross(cam.direction())).normalize() * .5135,
                          r, *c = new vec3[w * h];
